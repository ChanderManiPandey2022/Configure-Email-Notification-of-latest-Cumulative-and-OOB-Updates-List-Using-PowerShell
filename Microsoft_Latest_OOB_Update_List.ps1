...
$OutFileLP = "$WorkingFolder\MicrosoftOOBPatchList.csv";
$PatchingMonth = "";
$PatchReleaseDays = 0;
...
IF ($CollectedData) {
	$CollectedData = ($CollectedData | Where-Object {$_.class -eq "supLeftNavLink" -and $_.outerHTML -match "KB" -and $_.outerHTML -match "out-of-band" -and $_.outerHTML -notmatch "preview" -and $_.outerHTML -notmatch "mobile"}).outerHTML
}
...
Write-Host "Finalizing Patch List";
IF ($PatchingMonth) {
...
$output += "Latest OOB Update list for Windows 10/11</br>"
$output += '</br>'
$output += '<hr>'
$output += '<p>'
$output += "<h4>Hello All</h4>"
$output += "<h4>Please Find the  Latest OOB Update for Windows 10/11 update list released by Microsoft as on $(Get-Date -Format "yyyy-MM-dd").</h4>"
$output += '<p>'
